...
function renderWardens(){
 ...
 for(const w of S.wardens){syncTalentInputs(w);const defs=D.watchers[w.name]||[];const card=document.createElement('article');card.className='warden-card';
  ...
  defs.forEach((def,i)=>{const x=w.talents[i];const row=document.createElement('div');row.className='talent-row';row.innerHTML=`<div class="tal-name">${esc(def.attr)}<small> талант ${i+1}</small></div><div class="tal-star">×${def.star}<small>${Math.round((D.manuscriptChance[String(def.star)]||0)*1000)/10}% рук.</small></div><label>Ур. таланта<input inputmode="numeric" value="${esc(x.current)}" placeholder="1"></label><label>Рукописи<input inputmode="numeric" value="${esc(x.manuscripts)}" placeholder="0"></label><label>Свитки 100%<input inputmode="numeric" value="${esc(x.guaranteed)}" placeholder="0"></label><label class="tal-exp">Talent EXP<input inputmode="numeric" value="${esc(x.exp)}" placeholder="0"></label><div class="tal-result"><small>повысится</small><b data-plan>+0</b></div>`;const ins=row.querySelectorAll('input');['current','manuscripts','guaranteed','exp'].forEach((k,j)=>ins[j].addEventListener('input',e=>{x[k]=e.target.value;recalc()}));tg.append(row);});box.append(card);
 }
 setText('#wardenCount',`${S.wardens.length}/5`);$('#addWarden').disabled=S.wardens.length>=5;
}
...
function recalc(){
 ...
 $$('#wardenCards .warden-card').forEach((card,i)=>{const w=S.wardens[i];const calc=E.wardenCalc({name:w.name,currentLevel:n(w.currentLevel),targetLevel:n(w.targetLevel),talents:w.talents,inspiration});wGain+=calc.gain;wBlood+=calc.blood;card.querySelector('.w-blood').textContent=fmt(calc.blood);card.querySelector('.w-gain').textContent=calc.gainReady?fmt(calc.gain):'—';card.querySelector('.w-status').textContent=!calc.active?'заполни героя и уровни':!calc.gainReady?'введи текущие уровни всех талантов':'готово';card.querySelectorAll('[data-plan]').forEach((el,j)=>{el.textContent=calc.talents[j]?`+${calc.talents[j].planned}`:'+0'});});
